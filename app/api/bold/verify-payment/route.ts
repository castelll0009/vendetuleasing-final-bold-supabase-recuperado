...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[verify-payment] Admin client creado ✅")
    ...
    if (paymentError) {
      console.error("[verify-payment] ❌ ERROR al actualizar pago:", {
    ...
    console.log("[verify-payment] ✅ Pagos actualizados:", payments?.length || 0);
    console.log("[verify-payment] Datos devueltos:", payments);
    ...
    if (mappedStatus === "approved" && payment.property_id) {
      ...
      if (payment.payment_type === "publication") {
        ...
        if (propError) {
          ...
          console.log("[verify-payment] ✅ Propiedad publicada:", payment.property_id, "result:", propData)
        }
